    ...
    try:
        if platform == "yad2":
            ...
            if not settings.facebook_groups:
                click.echo(
                    "No Facebook groups configured. Please set FACEBOOK_GROUPS environment variable."
                )
                return
...
def list_listings(category, city, limit):
    ...
    try:
        ...
        for listing in listings:
            click.echo(
                f"- {listing.title} | {listing.price} {listing.currency} | {listing.city or 'Unknown'} | {listing.category}"
            )
...
def serve(host, port, reload):
    ...
    uvicorn.run("market_scout.api:app", host=host, port=port, reload=reload)
