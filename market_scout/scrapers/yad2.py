...
from market_scout.utils import extract_price, extract_city, normalize_text, categorize_product
...
class Yad2Scraper(BaseScraper):
    ...
    def search_listings(self, query: str, max_pages: int = 5) -> List[Dict[str, Any]]:
        ...
        for page in range(1, max_pages + 1):
            try:
                ...
                listing_elements = soup.find_all('div', class_=re.compile(r'feeditem|item|listing'))
                ...
                logger.info(f"Scraped page {page}, found {len(listing_elements)} listings")
    ...
    def _extract_listing_data(self, element) -> Optional[Dict[str, Any]]:
        ...
        try:
            # Extract title
            title_elem = element.find(['h2', 'h3', 'a'], class_=re.compile(r'title|link'))
